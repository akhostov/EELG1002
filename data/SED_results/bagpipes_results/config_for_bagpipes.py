...
sfr_timescale = 10**7  # This is 10 Myr
...
try:
    ...
    neb_cont_file = "/project/rlsa239_uksr/ali_ahmad/cloudy/bagpipes_models/Dopita00_abundances_ne_800/cloudy_temp_files/grids/bagpipes_nebular_cont_grids.fits"
    neb_line_file = "/project/rlsa239_uksr/ali_ahmad/cloudy/bagpipes_models/Dopita00_abundances_ne_800/cloudy_temp_files/grids/bagpipes_nebular_line_grids.fits"
    ...
    line_grid = [fits.open(neb_line_file)[i].data for
                 i in range(len(metallicities) * len(logU) + 1)]

    # Grid of nebular continuum fluxes.
    #cont_grid = [fits.open(grid_dir + "/" + neb_cont_file)[i].data for
    #             i in range(len(metallicities) * len(logU) + 1)]
    cont_grid = [fits.open(neb_cont_file)[i].data for
                 i in range(len(metallicities) * len(logU) + 1)]
